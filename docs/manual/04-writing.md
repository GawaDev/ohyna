...
本製品のドキュメント形式は **Ohyna Markdown** です。Markdown を基礎にした独自プロファイルで、CommonMark や GitHub Flavored Markdown の完全準拠は標榜しません。表やタスクリストなど GFM で一般的な記法の一部は使えますが、GitHub と同一の HTML になることは保証しません。

コードの色分け・数式・図は製品が定めた経路で処理します（数式は KaTeX 0.16.22、図は Mermaid.js メジャー 11）。キーボードキー表記（`<kbd>`）や検査の条件など、製品独自の事項は本章と「確認と PDF 出力」の章に記載します。
