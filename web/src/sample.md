...
ohyna:
  cover: true
  title: Ohyna 記法総合サンプル
  subtitle: Ohyna Markdown・準拠の考え方・記法確認
  label: SAMPLE
  meta:
    - Ohyna 1.0
    - 記法・準拠確認資料
...
本資料は、本製品の文書形式（**Ohyna Markdown**）で使える記法、コードの色分け、数式、Mermaid ダイアグラム、および**準拠・製品契約の考え方**を一通り確認するためのサンプルです。表紙および体裁は「ドキュメント設定」から変更できます。

> 印刷・配布向けの体裁確認と、製品契約の把握を目的としています。詳細は画面「ヘルプ」内の「仕様書」を参照してください。`[TOC]` は前後で改ページされます。
...
Ohyna が扱う入力は、一般的な Markdown の寄せ集めではなく、製品が定義する文書形式 **Ohyna Markdown** です。
...
- CommonMark／GFM の**完全準拠は標榜しません**。
- 表・タスクリスト・自動リンクなどは **GFM 由来の記法互換**として使えますが、GitHub と同一の HTML／表示互換は保証しません。
- 「ヘルプ」の「仕様書 → 準拠仕様マップ」が、競合時の優先順位と境界の基準です。
